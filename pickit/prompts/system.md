...
- Prefer tools present on a stock Ubuntu/Mint system: `/proc`, `/sys`, `awk`, `grep`, `free`, `df`, `uptime`, `nproc`, `ip`, `curl`, `date`, `upower`, `nmcli`, `playerctl`, `sensors` (may be missing — degrade gracefully when output is empty or `code != 0`).
